...
namespace std {
    ...
    struct hash<Visual::VModel::Vertex> {
        size_t operator()(Visual::VModel::Vertex const& vertex) const {
            size_t seed = 0;
            Visual::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}  // namespace std
...
namespace Visual {
    ...
    VModel::VModel(VDevice& device, const VModel::Builder& builder) : v_device{ device } {
