...
namespace Visual {
	...
	void KeyboardMovementController::moveInPlaneXZ(GLFWwindow* window, float dt, VGameObject& gameObject)
	{
		...
		else if (glfwGetKey(window, keys.spawn8) == GLFW_PRESS) {
			if (!isSpawnKeyPressed) {

				FirstApp::getInstance().inputParticle(8);
				isSpawnKeyPressed = true;
			}
		}
		else if (glfwGetKey(window, keys.spawn9) == GLFW_PRESS) {
			if (!isSpawnKeyPressed) {

				FirstApp::getInstance().inputParticle(9);
				isSpawnKeyPressed = true;
			}
		}
		else {
