...
namespace Physics {
    ...
    void ParticleDrag::UpdateForce(Particule *particule, float duration){
        auto vitesse = particule->getVitesse();
        auto norme_vitesse = sqrt(pow(vitesse.x, 2) + pow(vitesse.y, 2) + pow(vitesse.z, 2));
        auto force = ((vitesse.magnitude() > 10e-6f)? vitesse.normalized() : Physics::Vecteur3D()) * (m_k1 * norme_vitesse + m_k2 * pow(norme_vitesse, 2));
